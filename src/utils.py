...
class Park_classifier():
    ...
    def classify(self, image:np.ndarray, prosessed_image:np.ndarray,threshold:int=900)->np.ndarray:
        ...
        ###----    cv2.imshow(str(row_start*col_start),crop)
    ...
    def implement_process(self, image:np.ndarray)->np.ndarray:
        ...
        cv2.imshow("ImageConverted",gray)
        
        # gaussian bluring to reduce noise
        blur=cv2.GaussianBlur(gray, (3,3), 1)
        
        cv2.imshow("Imageblurr",blur)     
        ...
        cv2.imshow("Image Threshold",Thresholded)
        ...
        cv2.imshow("Imageblurr median",blur)  
...
class Coordinate_denoter():
    ...
    def read_positions(self)->list:
        ...
        -------
        list
            List of the tuples which stores the top left point coordinates of rectangle of car park:  [(x_1, y_1), ..., [x_n, y_n]]
        """
